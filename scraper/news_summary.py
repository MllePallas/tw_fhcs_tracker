...
# 2026-09 起排程一天三跑（07:07 / 16:07 / 19:07），上限按跑數換算以維持天數容忍：
# 預設 9 次 ≈ 3 天；大型金控媒體必定報導，撲空幾乎都是 web_search 索引延遲（偽陰性），
# 故拉高到 18 次 ≈ 6 天，避免新聞還沒被索引時就提前永久放棄。
NEWS_RETRY_CAP_DEFAULT = 9
...
- **金控合併層級的加計FVOCI數（115/06 起）**：富邦、凱基、國泰的新聞稿除壽險子公司外，
  亦揭露**金控合併層級**的加計FVOCI後獲利（國泰以「對保留盈餘影響數」表述金控層級累計數）。
  新聞有提到時，請在 **重點** 段落一併寫出金控層級的當月／累計加計數（例：「金控6月本期淨利
  加計FVOCI權益工具稅後處分利益292.2億元，累計前6月共1,884.1億元」），並區分清楚是金控還是壽險的數字
