...
> `.claude/settings.json` 已設 PreToolUse hook 攔截對本檔的 `Write`。提交前仍請 `git diff -- CLAUDE.md` 確認只有預期的增修。
...
│   ├── validate.py             # 資料一致性驗證（算術護欄，攔截 LLM 誤讀）
...
- 子公司、`fvoci_adjusted` 的 YoY 欄位亦套用同樣三件組（`yoy_pct` / `yoy_abs` / `yoy_status`）——**含金控層級** `holding_company.fvoci_adjusted`（baseline＝去年金控原始累計 P&L；lower_bound 同樣跳過）
...
## 加計 FVOCI 後獲利（fvoci_adjustment.py）——壽險子公司＋金控合併兩層級

2026 年起壽險公司接軌 IFRS 17，FVOCI 股票處份利益不再計入 P&L，使得 P&L 與去年同期（仍含 FVOCI 計入 P&L）難以直接比較。各金控會在月損益新聞稿揭露「**加計 FVOCI 處分利益後的獲利**」（當月＋累計），以利同基比較——**兩個層級**：

- **A. 壽險子公司層級**（自 115 年初起）：富邦人壽、凱基人壽（具體數字）、國泰人壽（門檻型「對保留盈餘影響數突破X億」）
- **B. 金控合併層級**（**115/06 起新增**）：`HOLDING_FVOCI_CODES = {2881, 2882, 2883, 2891}`。富邦／凱基揭露「金控稅後淨利加計FVOCI後獲利」具體數字；國泰以「**對保留盈餘影響數**」表述金控層級累計數（115/06 為**具體數字** 1,659 億，非門檻型）；**中信（2891）自 115/08 起揭露金控層級**（當月＋累計，新聞稿用語「金控本期淨利加計FVOCI權益工具稅後處分利益」，另揭露對保留盈餘影響之每股數 5.29 元 → 寫入 `cumulative_eps`），但**壽險層級（台灣人壽）仍不揭露**。台新新光（2887）目前不揭露金控層級，未來開始揭露時把代號加入 `HOLDING_FVOCI_CODES` 即可

**115/06 三家金控層級新聞稿範例（prompt 內建為參考樣本）**：
- 富邦：「金控6月本期淨利加計FVOCI權益工具稅後處分利益292.2億元，累計前6月共1,884.1億元，超越歷年全年獲利表現，總計之每股稅後盈餘為13.17元」→ 當月 29220、累計 188410、`cumulative_eps: 13.17`
- 國泰：「加計FVOCI股票處分損益，累計上半年對保留盈餘之影響數達1,659億元」→ 累計 165900（exact）、`label: "對保留盈餘影響數"`、無當月數
- 凱基：「單月稅後獲利61.1億元…若加計6月份的FVOCI股票處分利益，合計達到154.47億元…上半年累計稅後獲利加計FVOCI股票處分利益則為677.75億元」→ 當月 15447、累計 67775

**用語變更（2026-07 起，重要）**：主管機關要求新聞稿**不得使用「調整後獲利」**一詞（與會計原則不符），各壽險公司自 115/06 損益新聞稿起改用新表述——凱基／富邦：「**加計FVOCI獲利**」；國泰：「**對保留盈餘影響數**」。prompt 搜尋關鍵字已同時涵蓋新舊用語（舊聞 fallback 仍搜「調整後獲利」）。**115/07 起凱基改用「對保留盈餘貢獻」**（金控與人壽層級皆同，例：「加計單月稅後獲利，合計對保留盈餘貢獻達75.99億元」）→ 金控層級 `label: "對保留盈餘貢獻"`。**115/08 起富邦的 MOPS 重訊亦改稱「對保留盈餘影響數」**（註3 明載係本期淨利加計 FVOCI 權益工具稅後處分損益）→ 金控層級 `label: "對保留盈餘影響數"`。

⚠️ **115/06 凱基人壽層級誤植（2026-08-14 修正）**：當時 ctee 報導行文「旗下凱基人壽若加計…合計達154.47億元…累計677.75億元」把**金控層級**數字掛在人壽名下，自動抓取照寫進人壽層級。115/07 凱基新聞稿分層揭露後驗算確認：677.75 億＝金控加計累計（284.4＋處分利益393.35；再加 115/07 單月 75.99＝753.74 ✓ 與新聞明文吻合），人壽 6 月底加計累計應約 482.5 億（549.6−67.1），且人壽處分利益不可能大於金控層級。已將 115-06.json 凱基人壽的 `fvoci_adjusted` **整筆移除**（人壽層級無可靠的獨立揭露數，不可用差額回推補寫）。**勿對 115/06 重跑 `fvoci_adjustment.py`**——會再抓到同一篇 ctee 文章把錯誤數字寫回。

```bash
cd scraper
python fvoci_adjustment.py              # 補齊最新月份（冪等，兩層級各自判斷缺漏）
...
**作用對象**：`LIFE_INSURANCE_CODES = {2881, 2882, 2883, 2887, 2891}`（壽險層級：旗下名稱含「人壽」之子公司；金控層級：僅 `HOLDING_FVOCI_CODES`）。其他金控不處理。⚠️ 兩個清單是**獨立**的：中信在 `LIFE_INSURANCE_CODES` 內但台灣人壽無壽險層級揭露，每月仍會照常搜一次（撲空計數器獨立累計）。

**抓取邏輯**（`process_company` 每家**單次 LLM 呼叫**同時回傳 `{"life": ..., "holding": ...}` 兩層級；`_apply_level` 各自驗證寫入）：
1. LLM 直接搜尋新聞中揭露的加計 FVOCI 後**累計**獲利（必要）與**當月**獲利（選填，新聞有揭露才抓；禁止用累計差額回推）（NT$m）。壽險層級寫入 `subsidiaries[人壽].fvoci_adjusted`、金控層級寫入 `holding_company.fvoci_adjusted`
2. **兩層級數字各自取自新聞明確揭露的句子**，不可互相套用、不可由金控差額推算（金控合併 P&L 含其他子公司、少數權益、內部抵銷）
3. 合理性檢查（兩層級相同）：加計累計 > 對應層級原始累計，否則 not_found；當月數同理（`>=`），驗證不過只丟當月數、不影響累計
4. 撲空計數器分層獨立：壽險 `life_sub.fvoci_not_found_count`、金控 `holding_company.fvoci_not_found_count`，各滿 3 次永久跳過（--force 重置判斷）
5. **來源日期防呆**（`_extract_source_date` / `_announcement_month_start`）：要求新聞發布日 ≥ 公告月份第一天（N 月資料於 N+1 月公告，例 115/05 → 須 ≥ 2026-06-01）。擋掉公告前的「事前預估／掌握」稿。**僅對 URL 帶日期的來源有效**（工商時報 ctee `/news/YYYYMMDD`）；cnyes/udn/ltn 的 URL 無日期 → 無法驗證、維持放行
6. **區間/門檻型（`value_type: "lower_bound"`）**：只揭露門檻而非具體數字時（例國泰人壽「突破1,000億」），存下界數值 + `display_prefix`（逾／突破／超過），**不寫 `delta_vs_original`、不算 YoY**。`main.compute_yoy` 偵測 `value_type=="lower_bound"` 自動跳過 YoY。注意：新聞給「達1,659億元」這類**精確數字**時是 exact，不因句中出現「突破歷史」等字樣誤判
7. **金控層級選填欄位**：`label`（新聞以「對保留盈餘影響數」表述時寫入，前端以此覆寫列標籤；富邦／凱基標準用語不寫）、`cumulative_eps`（新聞揭露加計後「總計EPS」時寫入，目前僅富邦）

### 手動補 FVOCI（manual_fvoci.py）

富邦的加計 FVOCI 後獲利常只揭露在 Yahoo 股市（allowed_domains 以外），`fvoci_adjustment.py` 可能抓不到；此時人工寫入：

```bash
cd scraper
# 壽險子公司層級（預設），具體數字（exact）：累計必填，當月選填（NT$m，億×100）
...
# 金控合併層級（--holding）；富邦另揭露加計後總計EPS 可用 --eps
...
# 金控層級・國泰「對保留盈餘影響數」（exact、無當月數；--label 覆寫前端列標籤）
...
- 自動重算 YoY（lower_bound 自動跳過；金控層級由 compute_yoy 一併處理）、同步 latest↔歸檔
- 合理性檢查：累計 > 對應層級原始累計（否則報錯退出）；當月同理
- **富邦人壽通常只揭露累計加計數、不揭露當月加計數** → `--monthly` 省略，前端當月顯示 `—`；**國泰金控層級**只揭露累計對保留盈餘影響數 → 同樣省略 `--monthly`
...
**輸出欄位**（壽險子公司物件下 nested object；金控層級同構，掛在 `holding_company` 下）：
```json
"fvoci_adjusted": {
  "cumulative_profit": 54960,          // NT$m，直接從新聞抓到的累計加計FVOCI後獲利
  "monthly_profit": 6710,              // NT$m，當月加計FVOCI後獲利（新聞有揭露才有，選填）
  "delta_vs_original": 44557,          // = cumulative_profit − 對應層級原始累計 P&L（純紀錄用）
  "yoy_pct": 1024.2,                   // 由 main.compute_yoy 填入（累計基準）
  "source_url": "https://money.udn.com/money/story/5613/...",
  "source_quote": "凱基人壽…7月份稅後純益12.01億元，加計FVOCI股票處分利益約55.1億元，單月對保留盈餘貢獻為67.1億元…合計對保留盈餘貢獻達549.6億元。",
  "original_value_text": "單月67.1億元、累計前七月549.6億元",
  "generated_at": "2026-08-14T..."
}
```

**金控層級（`holding_company.fvoci_adjusted`）額外選填欄位**：
```json
"fvoci_adjusted": {
  "cumulative_profit": 165900,
  "label": "對保留盈餘影響數",          // 國泰金控層級用語；前端以此覆寫列標籤（預設「加上FVOCI股票處分利益」）
  "cumulative_eps": 13.17,             // 加計後總計EPS（富邦揭露；顯示於金控總覽 FVOCI 列的累計EPS欄）
  ...
}
```

**YoY 語意（兩層級相同）**：今年加計 FVOCI 後的調整數 vs 去年同期 baseline 的**同層級**原始 P&L（去年仍含 FVOCI 計入 P&L）→ 兩邊皆「含 FVOCI 影響」，apples-to-apples。`main.compute_yoy` 自動計算並寫入 `fvoci_adjusted.yoy_pct`（壽險層級 baseline＝去年壽險子公司累計；金控層級 baseline＝去年金控累計）。
...
`parse_profit_announcement()` 設計上是「規則式 `TableParser` → LLM 兜底」兩層，但**實務上 13/13 每月都走 LLM**（115/01–06 的 `parse_method` 全為 `llm`）。原因：MOPS 重大訊息**把財務數字全部放在「說明」欄位的單一 `<td>` 純文字裡**，是空白對齊的 ASCII 表，整份 HTML 的 4 個 `<table>` 都只是版面框架（抬頭、發言人、免責聲明），沒有任何一個含數字。`TableParser` 找 `<table>` 表頭算欄位索引，結構上永遠不可能命中 —— 這不是 2025 改版弄壞的，是前提本來就不成立。

資料本身一直是對的（LLM 判讀正確），但**LLM 這條路沒有任何護欄**：若某月它看錯一欄（例如把「合併稅前」當成「母公司業主稅後」），現在沒有機制會攔下來。`validate.py` 提供不依賴 LLM 的算術檢查。

### 檢查項

| 代號 | 檢查 | 層級 | 說明 |
|------|------|------|------|
| C0 | 必填欄位 | warn | 金控層級 `monthly_profit` / `cumulative_profit` 不應缺漏 |
| C1 | **累計連續性** | warn | `累計[N] − 累計[N−1] ≈ 當月[N]`（金控 + 子公司）—— 主力檢查 |
| C2 | 一月起始 | warn | 一月的累計應等於當月 |
| C3 | 單位欄位 | warn | `unit` 應為 `百萬元`（攔單位未換算） |
...
驗證器**無法自動分辨「解析錯誤」與「公告本身重編」**，所以定位是**警告而非阻擋**。已人工對照 MOPS 原始公告確認非解析錯誤者登錄於此，仍會顯示但歸類「已知例外」，不計入未解釋異常 —— 否則每月重複狂叫會讓人直接忽略報告。

key 格式 `(代號, 檢查代號, 對象, 期別)`，期別可用 `"*"` 代表所有月份。目前兩案：

- **`5880` 合庫金（所有月份，C1）**：公告的**當月數是「合併」稅後淨利**（含非控制權益），**累計數則取註解揭露的「歸屬於母公司業主」數** → 兩者基準不同，差額即當月非控制權益（每月約 0.1~1.5 億）。累計取母公司業主是專案既定規則（見「EPS 欄位說明」），**不應為了讓檢查通過而改動資料**
- **`2887` 台新新光金 114/10、114/11（C1、C5）**：兩期 JSON 均忠實照抄公告註解的「歸屬於母公司」數字，但公告自己對不上（114/09 累計 260.1 + 114/10 當月 78.3 = 338.4 ≠ 公告的 338.0；114/10 累計 338.0 + 114/11 當月 51.9 = 389.9 ≠ 公告的 338.7）。改用「合併」基準亦對不上，六家子公司累計則全部連續 → 係台新新光合併後整併調整導致金控累計數重編

⚠️ **新增例外前務必先對照 MOPS 原始公告**。登錄時把查證到的具體數字寫進說明，否則例外會變成「把問題掃到地毯下」。
...
python validate.py --strict         # 有未解釋異常時 exit 1（CI 用）
```

`main.py` 跑完 `save_data()` 後會**自動執行當期驗證並印出報告**（非阻擋：驗證失敗不影響資料寫入與 commit，只在 log 出 warning）。

**目前狀態**：114/01 ~ 115/06 共 18 個期別、約 1000 個資料點，**未解釋異常 0 筆**（16 筆已知例外）。
...
# 5. 資料一致性驗證（main.py 已自動跑過當期；此處補跑確認 FVOCI 寫入後仍通過）
python validate.py
```
...
5. **`python validate.py` 的「未解釋異常」為 0 筆**。若有：對照 MOPS 原始公告確認 —— 是解析錯誤就修資料（`main.py --force --codes XXXX` 重爬），是公告本身重編就登錄 `KNOWN_EXCEPTIONS`。**不可因為「數字看起來差不多」就略過**。
...
# 資料一致性驗證
python validate.py --all        # 全期別回歸檢查
...
- **模式切換（2026-08-18 新增）**：「單月月報｜期間比較」（`setPageMode`，body 掛 `.pm-period`）；期間比較詳見下方專節
- 排序：代號 / 當月↓ / **當月 MoM↓** / 累計↓ / **累計 YoY↓** / 累計 EPS↓（單月模式；期間比較另有獨立選項，由 `setSortOptionsForMode` 依模式重建，`_sortMemo` 記住各模式的選擇）
...
- 主表格（金控視角，**9 欄**，2026-08-18 起）：代號、金控名、當月、**當月 MoM**、累計、**累計 YoY**、當月 EPS、累計 EPS、**公告日期**（連結至 MOPS 原始公告）；2881/2882/2883 主列下方有金控層級 FVOCI 加計列。2026-08 起移除「狀態」欄（13/13 已成常態，資訊量低）
...
### 月度 AI 分析報告（`docs/report.html`，2026-08 新增）

報告在**其他專案**產製，本專案只負責展示。頁面與資料 pipeline 完全解耦——不動 `docs/data/`、不動爬蟲。

```
docs/
├── report.html          # 報告頁（獨立頁面，沿用 style.css）
├── report.js            # 讀 ?period → 查 index.json → 取 Markdown → 渲染
└── reports/
    ├── index.json                      # 報告索引（唯一要手動維護的檔案）
    └── 2026-06_金控自結獲利分析.md      # 報告內容（Markdown），檔名可自訂
```

**每月新增一份報告只有兩步**（不需要改任何程式）：

1. 把 Markdown 檔放進 `docs/reports/`。**檔名可自訂**（`report.js` 直接用 index.json 的 `file` 欄位取檔，不解析檔名），只要與 `file` 欄位完全一致即可
2. 在 `docs/reports/index.json` 的 `reports` 陣列**最前面**加一筆（由新到舊）：
   ```json
   { "period": "115/07", "file": "2026-07_金控自結獲利分析.md", "title": "…", "source": "AI 生成" }
   ```

首頁「金控總覽」表格右下方會自動出現「AI 生成分析報告」按鈕（`renderReportLink()`，另開新頁）。索引裡沒有的月份不顯示按鈕；產業視角（銀行／壽險／證券）不顯示，因報告為全月綜觀。

**index.json 欄位**：`period`（必填，民國年/月）、`file`（必填）、`title`（選填，省略時取內文第一個 `#` 標題）、`generated_at`（選填 `YYYY-MM-DD`，會顯示於報告上緣）、`source`（選填，如「AI 生成」）、`draft`（選填，`true` 時頁面標示「排版範例」）。

**Markdown 支援範圍**（`renderMarkdown()` 自製，無外部相依）：`#`~`####` 標題（`#`/`##` 皆對應 h2）、段落、`**粗體**`、`*斜體*`、`` `程式碼` ``、`-`/`1.` 清單、`>` 引言、`---` 分隔線、`[文字](網址)`、表格（含 `---:` 右對齊）、反斜線跳脫（`\*`）。渲染前一律 `escapeHtml()`，報告內容不會被當成 HTML 執行。

**與站內視覺一致的處理**：
- 內文第一個 `#` 標題自動抽出當作頁面 h1，不會在內文重複出現
- 整段為斜體的行（如「*單位：新台幣億元…*`」）自動轉為小字說明列 `.report-caption`
- 表格中**純粹帶正負號的數字**（`+18.5%`、`-39.8%`，允許結尾 `*`／`†`）自動套用站內方向色（綠／紅）；帶文字的儲存格如「約 -5.3%」「不可比」維持墨黑，避免誤判
- 手機：報告表格可橫向捲動，整頁不溢出
...
### 期間比較模式與單月 MoM 拆解（2026-08-18 新增）

**模式切換**：controls 列新增「單月月報｜期間比較」segmented control（`setPageMode`，body 掛 `.pm-period`）。期間比較模式下以 CSS 隱藏單月專屬元件：月份選單、市場概況、下載 Excel、存成圖片、手機卡片切換、第二張圖（累計圖，容器 `#cumul-chart-container`）、AI 報告連結。

**期間選項（`buildPeriodOptions`）**：由 index.json 月份清單產生「季度＋年度」兩組，**不開放任意起訖月**。季度標籤如 `1Q26`；進行中的季度標示「（至N月）」，去年同期取**相同月份**比較。年度：12 個月俱全顯示「全年」，否則 `2026 YTD（1–N月）`。預設選最新**完整**季度。

**期間獲利計算（`periodAggCompany`）**：`期末累計 − 期初前月累計`（一月起算直接用期末累計），**刻意不用單月加總**。理由：(1) 合庫金單月為合併基準、累計為母公司業主基準（見 KNOWN_EXCEPTIONS），累計軋差才與全站累計欄位同基準；(2) 公告重編（如 2887 114/10-11）已反映在累計數。每個期間只需抓「期末」與「期初前月」兩檔（`ensurePeriodData`），與 MoM 共用 `monthCache`。子公司以名稱跨期對應，更名走 `SUB_PREV_ALIAS`（玉山投信 ← 保德信投信，114/09 更名同實體）；期初前月未列之子公司（如京城銀 114/10 併入）以 0 起算——已驗證新併入子公司的公告累計自併入月起算，此處理正確。期間 EPS＝累計 EPS 軋差之**推算值**（增資／換股月份或有失真），註腳載明。

**期間 YoY（`periodYoyOf`）**：本期期間數 vs 去年同期同法計算之期間數。跨零點沿用虧轉盈／盈轉虧＋絕對差額慣例——⚠️ 此時 `pct` 仍須帶原始計算值再交給 `formatYoY`（`formatYoY` 見到 `pct == null` 會直接回 `—`，status 分支根本走不到）。**2887 規則與正式站一致：期間「期末」< 115/07 → 顯示「基期為合併前」**（1Q26/2Q26 擋掉；3Q26 與 YTD26 可比——YTD26 算出 281.3% 與 main.py 預算的累計 YoY 完全一致，惟含合併基期效果）。2890 京城銀註記同以期末 < 115/07 判斷。114 年各期間無 113 年資料 → YoY 一律 `—`（優先於 2887 判斷，避免顯示無意義的合併註記）。**FVOCI 加計數不納入期間比較**（門檻型無法加總），註腳載明。

**期間比較 UI**：四產業 tab 共用（子公司同法軋差）。表格附**排名欄**（固定依期間獲利遞減，`rank-top` 標第一名）與「去年同期」欄；四張第一名卡片改為「{期間}獲利第一」；圖表僅一張（本期 vs 去年同期，缺基期列 chart-note；無基期年份顯示「無去年同期資料」）。點公司名開 `showPeriodDetail`：子公司期間拆解＋「母公司及其他（軋差）」列。手機：`view-period` 表格 min-width 720px 橫向捲動、名稱欄（**第 3 欄**）sticky 釘左（覆寫預設第 2 欄 sticky）。

**單月 MoM 欄（金控總覽）**：「合併稅後淨利」欄群 3→4 欄（**表格 8→9 欄**；error 列 colspan 5→6、FVOCI 加計列在 MoM 位置補一格空欄、tfoot colspan 8→9）；排序新增「當月 MoM↓」（`mom_desc`，tier 同 YoY：轉盈>正>負>轉虧）；手機卡片加第 5 格。`loadData` 時順帶抓上月檔（`loadPrevMonth` → `state.prevMonth`；一月的上月是去年十二月）。跨零顯示「轉盈／轉虧＋差額」；`MOM_INCOMPARABLE = { '2887': ['114/07'] }`（合併首月上月非同一實體 → `—`）。⚠️ **存成圖片與 Excel 匯出維持原 8 欄版型**（`snapBuildModel` / `buildHoldingsSheet` 未動）——若要加 MoM 欄，兩處需另行同步。

**單月獲利變動拆解（`momAttributionHtml`，詳情面板最上方）**：金控 上月→本月 增減額，逐子公司歸因（上月→本月、增減額、長條圖，依增減排序），差額列「母公司及其他（軋差）」（含母公司本身損益、合併沖銷與未揭露項目——實測各家每月軋差約 -10 億～+4 億，不可省略）。子公司「上月未列」（新併入，如京城銀 114/10）／「本月未列」（如台新人壽 115/01 併入新光人壽）以標籤標示、以 0 基準計增減。**設計決策：不顯示佔比**——子公司增減與金控增減方向相反時佔比為負、也可能 >100%，正負號語意誤導（2026-08-18 依使用者要求移除）；相對規模由長條圖表達。

**驗證（2026-08-18）**：全部期間數與獨立以 Python（累計軋差）重算之值吻合；YTD26 期間 YoY 與 main.py 預算的 `cumulative_profit_yoy_pct` 13/13 一致；MoM 拆解「子公司增減＋軋差＝金控增減」逐列驗算通過；Playwright 桌機／390px 手機無 console error。
...
- **`parser.py` 的 `TableParser` 實質上是死碼**：MOPS 重大訊息把數字放在「說明」欄的純文字裡，沒有任何 `<table>` 含數字 → 規則層永遠回傳 `None`，13/13 每月都走 LLM。**資料是對的**（LLM 判讀正確，已逐欄對照原始公告驗證），只是每月固定付 13 次 Opus 解析費。護欄見 `validate.py`。若要真正啟用規則解析，需改寫成「說明文字區塊解析器」＋各家欄位樣板（各家欄位語意不同：富邦給「母公司業主稅後」、兆豐給「當月稅後」、合庫當月是合併數）
- **富邦金的重訊本身就含 FVOCI 區塊**（金控與人壽的加計後當月/累計/EPS 都在裡面），理論上不必靠 `fvoci_adjustment.py` 搜新聞；但**國泰、凱基到 115/06 為止的重訊未揭露 FVOCI**，仍須依賴新聞或 `manual_fvoci.py`
- **早鳥金控會沿用到上個月的新聞摘要（2026-09-14 修正）**：`main.py` 合併既有資料時，「保留 `news_summary` / `news_sources` / `news_generated_at`」那段原本**沒有檢查期別**，而 `existing` 來自 `latest.json`。月初第一跑（8 日）時 `latest.json` 仍指上個月 → 當天剛公告的早鳥（玉山／國票／永豐）財報數字是新的，新聞摘要卻被填上**上個月的內容**，`news_generated_at` 也停在上個月。因 `news_summary.py` 見到已有摘要就跳過，錯誤會一路留到月底。已加 `existing_same_period` 守門（同下方 cache 分支的判斷）。歷史受害：115/08 的 2884／2889／2890、115/04 的 2884（已重生成 115/08 三家；115/04 因新聞已過索引期未動）。**日後若發現某家摘要的月份對不上財報數字，先比對 `news_generated_at` 與公告日期**
