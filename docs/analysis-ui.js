...
function appendAnalysisSheets(wb,XLSX) {
  ...
  sheet('新聞摘要',['月份','代號','公司','摘要','摘要生成時間','人工整理','證據層級','來源識別'],pack.news.map(r=>[txt(r.period),txt(r.code),txt(r.name),txt(r.summary),txt(r.generated_at),txt(r.manual?'是':'否'),txt('二次整理，需核對原文'),txt(r.id)]),[15,10,18,110,28,15,35,35]);
